...
const RegisterPage: React.FC = () => {
  ...
  const [chaukiName, setchaukiName] = useState('');
  const [chaukiID, setchaukiID] = useState('');
  const [password, setPassword] = useState('');
  const [error, setError] = useState('');
  const [chaukiIDError, setchaukiIDError] = useState('');
  const [success, setSuccess] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [isCheckingId, setIsCheckingId] = useState(false);
  const [showPassword, setShowPassword] = useState(false);

  const validatechaukiID = async (id: string): Promise<boolean> => {
    if (!id) return false;
    try {
      const thanaRef = collection(db, 'police_stations');
      const q = query(thanaRef, where('chaukiID', '==', id));
  ...
  const handlechaukiIDChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (id.length >= 3) {
      ...
      try {
        const isUnique = await validatechaukiID(id);
        if (!isUnique) {
          setchaukiIDError('This Thana ID already exists');
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Validation checks
      if (!chaukiName || !chaukiID || !password) {
      ...
      const isUnique = await validatechaukiID(chaukiID);
      if (!isUnique) {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input
              type="text"
              value={chaukiName}
              onChange={(e) => setchaukiName(e.target.value)}
              className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none transition-all ${error && !chaukiName ? 'border-red-500' : 'border-gray-300'
                }`}
          ...
          <div>
            ...
            <div className="relative">
              <input
                type="text"
                value={chaukiID}
                onChange={handlechaukiIDChange}
                className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none transition-all ${(error && !chaukiID) || chaukiIDError ? 'border-red-500' : 'border-gray-300'
                  }`}
            ...
            {chaukiIDError && (
              <p className="mt-1 text-sm text-red-600">{chaukiIDError}</p>
            )}
          </div>
...
<p className="text-gray-600 mt-2"><a href="/login">Already registered your chauki?</a></p>
          <button
            ...
            className={`w-full bg-indigo-600 text-white py-2 rounded-lg font-medium
              ...
              ${(isLoading || isCheckingId || !!chaukiIDError) ? 'opacity-75 cursor-not-allowed' : ''}`}
          >
