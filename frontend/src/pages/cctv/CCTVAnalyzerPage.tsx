    ...
    return (
      <div className="relative w-full h-full rounded-xl overflow-hidden bg-gray-800 flex items-center justify-center">
        ...
        {onRemove && (
          <button
            onClick={onRemove}
            className="absolute top-2 right-2 p-2 bg-red-500 rounded-full text-white hover:bg-red-600 transition-colors z-10"
          >
            <X className="w-4 h-4" />
          </button>
        )}
  ...
  return (
    ...
    >
      ...
      {selectedFile ? (
        <div className="text-center">
          ...
          {onRemove && (
            <button
              ...
              className="mt-4 p-2 bg-red-500 rounded-full text-white hover:bg-red-600 transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
          )}
        ...
        <>
          <div className="flex items-center justify-center w-20 h-20 mb-4 rounded-full bg-gray-700 transition-transform group-hover:scale-110">
            <Upload className="w-10 h-10 text-gray-400" />
          </div>
          <p className="text-gray-300 font-medium mb-2">
            Click to upload {type === "video" ? "video footage" : "suspect image"}
          </p>
          <p className="text-gray-500 text-sm mb-2">or drag and drop</p>
          <p className="text-gray-400 text-xs">Supported formats: {acceptText}</p>
        </>
      )}
      {error && (
...
const ProcessingDisplay: React.FC<ProcessingDisplayProps> = ({ progress }) => (
  <motion.div
    initial={{ opacity: 0, y: 20 }}
    animate={{ opacity: 1, y: 0 }}
    exit={{ opacity: 0, y: -20 }}
    className="bg-gray-800/50 rounded-xl p-8 flex flex-col items-center"
  >
...
const CCTVAnalyzerPage: React.FC = () => {
  ...
  const handleSuspectImageSelect = (file: File) => {
    ...
    reader.onloadend = () => setSuspectImagePreview(reader.result as string);
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (!videoToUse) throw new Error("No video selected");
      ...
      if (!analyzeResponse.ok) throw new Error("Failed to start analysis");
  ...
  const groupedResults = React.useMemo(() => {
    return Object.entries(
      results.reduce((acc, result) => {
        if (!acc[result.suspectName]) acc[result.suspectName] = [];
      ...
      <div className="flex-1">
        <div className="max-w-6xl mx-auto px-4 py-8">
          ...
          <div className="flex flex-col gap-8">
              ...
              {!isAnalyzing ? (
                activeTab === "existing" ? (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: -20 }}
                    className="bg-gray-800/60 rounded-xl p-8 shadow-lg"
                  >
                    <FileUpload
                  ...
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: -20 }}
                    className="bg-gray-800/60 rounded-xl p-8 shadow-lg flex flex-col gap-8"
                  >
                ...
                <ProcessingDisplay progress={analysisProgress} />
              )}
            </AnimatePresence>
            {/* Action Buttons */}
            {!isAnalyzing && (
            ...
            {/* Error Message */}
            {error && (
              <div className="max-w-xl mx-auto mt-4 p-4 bg-red-500/20 border border-red-500 rounded-lg flex items-center gap-2">
                <AlertCircle className="w-5 h-5 text-red-500" />
            ...
            {groupedResults.length > 0 && (
              <div className="bg-gray-800/60 rounded-xl p-6 shadow-lg mt-8">
                  ...
                  <div>
                    ...
                    {stats && (
                      <div className="mt-1 text-sm text-gray-400">
                        <p>
                          Processed {stats.framesProcessed.toLocaleString()} frames in {processingTime?.toFixed(2)} seconds
                        </p>
